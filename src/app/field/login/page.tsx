  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-sm flex-col justify-center gap-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-600">Field crew access</p>
      </div>
      {error ? (
      ...
      <form action="/api/auth/field" method="POST" className="flex flex-col gap-4">
        <input type="hidden" name="next" value={next ?? "/field"} />
        <label className="flex flex-col gap-2">
          <span className="text-sm font-medium">Crew passcode</span>
          <input
            type="password"
            name="passcode"
            inputMode="text"
