...
export function FieldShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isLogin) return;
    // §9: sync on regained connectivity and on app foreground. Never blocks
    // the UI; anything that fails stays queued.
        ...
        <div className="flex items-center gap-2 text-sm">
          <span
            className={`flex items-center gap-1 rounded-full px-2 py-1 font-medium ${
              online
                ? "bg-green-100 text-green-800 hc:bg-black hc:text-green-300"
                : "bg-neutral-200 text-neutral-700 hc:bg-black hc:text-yellow-300"
            }`}
          >
            <span
              aria-hidden
          ...
          <Link
            href="/field/queue"
            className="rounded-full bg-neutral-100 px-3 py-1 font-medium hc:border hc:border-yellow-400 hc:bg-black"
          >
            {pending} pending
          </Link>
          <button
